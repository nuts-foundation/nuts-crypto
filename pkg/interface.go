...
type Client interface {
	...
	// StoreCertificate stores the given certificate for the given key. If there's a private key present, it must match
	// the public key (otherwise an error is returned). If there's an existing certificate entry, it will be overridden.
	// This is typically used to store a certificate resulting from the CSR generated by GenerateVendorCACSR.
	StoreCertificate(key types.KeyIdentifier, certificate *x509.Certificate) error
	...
	// GetSigningCertificate retrieves a signing certificate which is issued under the given key. If there's no signing certificate for the
	// given entity or it has expired, a new certificate will be issued. As such, the key MUST be a Certificate Authority
	// or otherwise an error will be returned.
	//
